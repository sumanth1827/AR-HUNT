       ...
       if (score <= 10)
        {
            ...
            tmp.GetComponentInChildren<score12>().n = 8;
        }
        else if(score>10)
        {
            obs.GetComponent<obstacle>().speed = 18f;
            tmp.GetComponentInChildren<score12>().n = 12;

            if (score > 15)
            {
                obs.GetComponent<obstacle>().speed = 20f;
                tmp.GetComponentInChildren<score12>().n = 15;
                if (score > 20)
                {
                    obs.GetComponent<obstacle>().speed = 25f;
                    tmp.GetComponentInChildren<score12>().n = 25;
                    if (score > 25)
                    {
                        obs.GetComponent<obstacle>().speed = 30f;
                        tmp.GetComponentInChildren<score12>().n = 32;
